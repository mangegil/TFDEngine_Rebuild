...
// Forward declare RE::InputEvent (used by SMF input callbacks)
namespace RE
{
    class InputEvent;
}

// Lazy-load SMF module handle (use W explicitly)
inline HMODULE menuFramework = nullptr;

inline HMODULE EnsureMenuFrameworkLoaded()
{
    if (!menuFramework) {
    ...
    namespace Model {
        namespace Internal {
            ...
            T GetFunction(LPCSTR name) {
                auto mod = EnsureMenuFrameworkLoaded();
                if (!mod) {
                    return nullptr;
                }
